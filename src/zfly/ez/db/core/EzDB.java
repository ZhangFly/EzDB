...
import zfly.ez.db.sql.builder.*;
import zfly.ez.db.strategy.reflect.ReflectTableImpl1;
import zfly.ez.db.strategy.reflect.ReflectTableObserver;
import zfly.ez.db.strategy.reverse.ReverseTableDelegate;
import zfly.ez.db.strategy.reverse.ReverseTableImpl1;
import zfly.ez.db.strategy.reverse.ReverseTableStrategy;
...
public class EzDB {
	...
	private static Logger log = Logger.getLogger(EzDB.class);

	private EzDBConfig config;
	private EzSQLExecutor sqlExecutor;
	 ...
	 * @throws SQLException
	 */
	public static EzDB createDB(final EzDBConfig config) throws SQLException {
	 ...
	 * @throws SQLException
	 */
	public static EzSQLExecutor createSQLExecutor(final EzDBConfig config) throws SQLException {
		...
		return new EzSQLExecutor(config.getPoolSize(), config.getUrl(), config.getUserName(), config.getPassWord());
	}
	...
	private void executeSQL(final SQLBuilder sqlBuilder, final EzDBHandler handler) throws SQLException {
